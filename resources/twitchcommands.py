...
def cd_check(id):
	...
	if (time.time() - last_used[id]) > cooldowns[id]:
		...
		return True
	elif COOLDOWNMESSAGE == "true":
		sendMessage(irc, "@"+user+" Command "+command_names[id]+" is on cooldown ("+str(int(last_used[id]-(time.time()-cooldowns[id])))+"s left)")
		message = ""
		return False
	else:
		message = ""
...
def gamecontrol():
	...
	while True:
		...
		if PREFIX + "superjump" == str(args[0]).lower() and cd_check(1):
		...
		if (PREFIX + "superboosted" == str(args[0]).lower() or PREFIX + "superboosteds" == str(args[0]).lower()) and cd_check(2):
			sendForm("(if (not(=(-> *edge-surface* fric) 1.0))(set! (-> *edge-surface* fric) 1.0)(set! (-> *edge-surface* fric) 30720.0))")
			message = ""

		if (PREFIX + "noboosteds" == str(args[0]).lower() or PREFIX + "noboosted" == str(args[0]).lower()) and cd_check(3):
			sendForm("(if (not(=(-> *edge-surface* fric) 1530000.0))(set! (-> *edge-surface* fric) 1530000.0)(set! (-> *edge-surface* fric) 30720.0))")
			message = ""
			
		if PREFIX + "fastjak" == str(args[0]).lower() and cd_check(4):
		...
		if PREFIX + "slowjak" == str(args[0]).lower() and cd_check(5):
		...
		if PREFIX + "pacifist" == str(args[0]).lower() and cd_check(6):
		...
		if PREFIX + "trip" == str(args[0]).lower() and cd_check(7):
			sendForm("(send-event *target* 'loading)")
			message = ""
			
		if PREFIX + "shortfall" == str(args[0]).lower() and cd_check(8):
		...
		if PREFIX + "ghostjak" == str(args[0]).lower() and cd_check(9):
			sendForm("(set! (-> *TARGET-bank* body-radius) (meters -1.0))")
			time.sleep(3)
			sendForm("(set! (-> *TARGET-bank* body-radius) (meters 0.7))")
			message = ""
			
		if PREFIX + "getoff" == str(args[0]).lower() and cd_check(10):
			sendForm("(send-event *target* 'end-mode)")
			message = ""
			
		if (PREFIX + "setflutflut" == str(args[0]).lower() or PREFIX + "flutspeed" == str(args[0]).lower()) and len(args) >= 2 and cd_check(11):
			sendForm("(set! (-> *flut-walk-mods* target-speed)(meters " + str(args[1]) + "))")
			message = ""
			
		if PREFIX + "freecam" == str(args[0]).lower() and cd_check(12):
			sendForm("(stop 'debug)")
			time.sleep(6)
			sendForm("(start 'play (get-or-create-continue! *game-info*))")
			message = ""
			
		if PREFIX + "enemyspeed" == str(args[0]).lower() and len(args) >= 3 and cd_check(13):
			sendForm("(set! (-> *" + str(args[1]) + "-nav-enemy-info* run-travel-speed) (meters " + str(args[2]) + "))")
			message = ""
			
		if PREFIX + "give" == str(args[0]).lower() and len(args) >= 3 and cd_check(14):
			sendForm("(set! (-> *game-info* " + str(args[1]) + ") (+ (-> *game-info* " + str(args[1]) + ") " + str(args[2]) + "))")
			message = ""
			
		if (PREFIX + "setcollected" == str(args[0]).lower() or PREFIX + "collected" == str(args[0]).lower()) and len(args) >= 3 and cd_check(15):
			sendForm("(set! (-> *game-info* " + str(args[1]) + ") (+ 0.0 " + str(args[2]) + "))")
			message = ""

		if PREFIX + "eco" == str(args[0]).lower() and len(args) >= 2 and cd_check(16):
			sendForm("(send-event *target* 'get-pickup (pickup-type eco-" + str(args[1]) + ") 5.0)")
			message = ""
			
		if (PREFIX + "sucksuck" == str(args[0]).lower() or PREFIX + "setsucksuck" == str(args[0]).lower()) and len(args) >= 2 and cd_check(17):
			sendForm("(set! (-> *FACT-bank* suck-suck-dist) (meters " + str(args[1]) + "))(set! (-> *FACT-bank* suck-bounce-dist) (meters " + str(args[1]) + "))")
			message = ""
			
		if PREFIX + "noeco" == str(args[0]).lower() and cd_check(18):
		...
		if PREFIX + "die" == str(args[0]).lower() and cd_check(19):
			sendForm("(initialize! *game-info* 'die (the-as game-save #f) (the-as string #f))")
			message = ""
			
		if (PREFIX + "gotolevel" == str(args[0]).lower() or PREFIX + "gotopoint" == str(args[0]).lower()) and len(args) >= 2 and cd_check(20) and ALLPOINTS == "true":
			sendForm("(start 'play (get-continue-by-name *game-info* \"" + str(args[1]) + "\"))")
			message = ""
			
		if (PREFIX + "randompoint" == str(args[0]).lower() or PREFIX + "randomcheckpoint" == str(args[0]).lower()) and cd_check(21):
			sendForm("(start 'play (get-continue-by-name *game-info* \"" + str(random.choice(point_list)) + "\"))")
			message = ""
			
		if PREFIX + "tp" == str(args[0]).lower() and len(args) >= 4 and cd_check(22):
			sendForm("(set! (-> (target-pos 0) x) (meters " + str(args[1]) + "))  (set! (-> (target-pos 0) y) (meters " + str(args[2]) + ")) (set! (-> (target-pos 0) z) (meters " + str(args[3]) + "))")
			message = ""
			
		if PREFIX + "shift" == str(args[0]).lower() and len(args) >= 4 and cd_check(23):
		...
		if PREFIX + "movetojak" == str(args[0]).lower() and len(args) >= 2 and cd_check(24):
		...
		if PREFIX + "deload" == str(args[0]).lower() and cd_check(33):
			sendForm("(set! (-> *load-state* want 0 display?) #f)")
			message = ""
			
		if PREFIX + "frickstorage" == str(args[0]).lower() and cd_check(34):
			sendForm("(stop 'debug)", 34)
			time.sleep(0.001)
			sendForm("(start 'debug (get-or-create-continue! *game-info*))")
			message = ""
			
		if PREFIX + "dark" == str(args[0]).lower() and cd_check(35):
		...
		if PREFIX + "dax" == str(args[0]).lower() and cd_check(36):
			sendForm("(send-event *target* 'sidekick (not (not (send-event *target* 'sidekick #t))))")
			message = ""
			
		if PREFIX + "smallnet" == str(args[0]).lower() and cd_check(37):
		...
		if PREFIX + "moveplantboss" == str(args[0]).lower() and cd_check(40):
		...
		if PREFIX + "moveplantboss2" == str(args[0]).lower() and cd_check(41):
			sendForm("(set! (-> *pc-settings* force-actors?) #t)")
			time.sleep(0.050)
			sendForm("(when (process-by-ename \"plant-boss-3\")(set-vector!  (-> (-> (the process-drawable (process-by-ename \"plant-boss-3\"))root)trans) (meters 436.97) (meters -43.99) (meters -347.09) 1.0))")
			message = ""
			
		if PREFIX + "basincell" == str(args[0]).lower() and cd_check(42):
		...
		if PREFIX + "resetactors" == str(args[0]).lower() and cd_check(43):
		...
		if str(args[0]) == PREFIX + "repl" and len(args) >= 2 and cd_check(44):
